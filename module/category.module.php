...
class category_module extends admin_module {
    ...
    public function list_action () {
        ...
        $tab = RGX\OBJ('category_table')->where("cat_type = {$type_id}")->where('cat_admin_id = ' . $this->login['admin_id']);
    ...
    public function add_action () {
        ...
        // 新增
            ...
            if (!empty($parent)) {
                ...
                $type = RGX\category_helper::$type[$cat['cat_type']];
                $this->set_pos([
                    '@category-list-type-' . $type_id   => "{$type}",
                    'cur'   => "新增{$type}"
                ]);
            }
            else {
    ...
    public function save_action () {
        ...
        $cat['cat_admin_id'] = $this->login['admin_id'];
        if ($tab->load($cat)) {
            if ($cat['cat_id'] > 0) {
