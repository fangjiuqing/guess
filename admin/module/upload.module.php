...
class upload_module extends admin_module {
    ...
    public function image_action () {
        ...
        if ($out['data']['error']) {
            ...
            RGX\image::get_instance()->thumb($sfile, $sfile, '1800xauto');
            RGX\image::get_instance()->thumb($sfile, '900x600');
            RGX\image::get_instance()->thumb($sfile, '1000x1414');
            RGX\image::get_instance()->thumb($sfile, '500x309');
            RGX\image::get_instance()->thumb($sfile, '300x300');
        }
        $this->ajaxout($out);
    }
